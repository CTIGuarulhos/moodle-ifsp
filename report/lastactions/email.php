...
foreach($usersemail as $useremail){
	...
	<head>
	<title></title>
		<link rel="stylesheet" href="'.$CFG->wwwroot.'/report/lastactions/email.css">
	</head>
	<body>';
	...
	foreach($reportcmids as $courseid => $reportcmid){
		...
		$lastactionstable->head = array($stractivity.'/'.$struser,$strevent, $strviews);
		...
		$saida .= html_writer::table($lastactionstable);
	}
	$saida .= '</body></head>';
	...
	if(count($reportcmids)>0){
		...
		// message
		$message = '<p align="center"><img src="http://moodle.bento.ifrs.edu.br/report/lastactions/top_moodle.png" width="100%"><br><h3 align="center">Confira as últimas atualizações em seus cursos do Moodle IFRS-BG</h3></p>'.$saida;
