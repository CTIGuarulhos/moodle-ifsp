...
require_once($CFG->dirroot.'/report/role/lib.php');

$userid   = required_param('id', PARAM_INT);
$courseid = required_param('course', PARAM_INT);
$mode     = optional_param('mode', 'role', PARAM_ALPHA);

if ($mode !== 'complete' and $mode !== 'role') {
    $mode = 'role';
}
...
$PAGE->set_url('/report/role/user.php', array('id'=>$userid, 'course'=>$courseid, 'mode'=>$mode));

if (!report_role_can_access_user_report($user, $course, true)) {
    require_capability('report/role:view', $coursecontext);
}

$stractivityreport = get_string('activityreport');

$PAGE->set_pagelayout('report');
$PAGE->set_url('/report/role/user.php', array('id'=>$user->id, 'course'=>$course->id, 'mode'=>$mode));
...
$event = \report_role\event\report_viewed::create(array('context' => context_course::instance($course->id),
            ...
            if (!empty($modinfo->sections[$i])) {
                ...
                foreach ($modinfo->sections[$i] as $cmid) {
                    ...
                    if (file_exists($libfile)) {
                        ...
                        switch ($mode) {
                            case "role":
                                $user_role = $mod->modname."_user_role";
                                if (function_exists($user_role)) {
                                    $output = $user_role($course, $user, $mod, $instance);
                                } else {
                                    $output = report_role_user_role($user->id, $cmid, $mod->modname, $instance->id);
                                }
                                report_role_print_row($mod, $instance, $output);
                                break;
                            case "complete":
                                ...
                                if (function_exists($user_complete)) {
                                    ...
                                    echo report_role_user_complete($user->id, $cmid, $mod->modname, $instance->id);
                                }
                                echo "</ul>";
