...
if ($useinternalreader) {
    ...
    $sql = " SELECT l.id, contextinstanceid as cmid, l.userid, count(1) AS numviews
	...
	if(count($v)>0){
        foreach ($v as $key => $value) {
			$views[$value->cmid][$value->userid]['read'] = $value->numviews;
        }
	}
	
	$sql = " SELECT l.id, contextinstanceid as cmid, l.userid, count(1) AS numpart
              FROM {logstore_standard_log} l
             WHERE courseid = :courseid
               AND anonymous = 0
               AND crud = 'c'
	...
	if(count($v)>0){
        foreach ($v as $key => $value) {
			//$views[$value->cmid][$value->userid]['read'] = $value->numviews;
			$views[$value->cmid][$value->userid]['action'] = $value->numpart;
			/*if($value->numpart == 'yes' && $value->numviews == 0){
				echo $value->cmid.'-'.$value->userid.'<br>';
			}*/
        }
	}
}
    ...
    foreach ($section as $cmid) {
		...
		//if(in_array($cmid, $show)){
			...
			foreach($students as $t){
				...
				$r = "0";
				$a = "";
				if(isset($views[$cm->id][$t]['read'])){
					$r = $views[$cm->id][$t]['read'];
				}
				if(isset($views[$cm->id][$t]['action'])){
					if(isset($show[$cm->id]))
						$a = "/".get_string('yes');
				}
				$numviewscell->text = $r.$a;
...
echo 'Legenda:<br> Quantidade de visualizações / Participou';
