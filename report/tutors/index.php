...
require('../../config.php');
require_once($CFG->dirroot.'/report/tutors/locallib.php');

$id = required_param('id',PARAM_INT);       // course id

$course = $DB->get_record('course', array('id'=>$id), '*', MUST_EXIST);

$PAGE->set_url('/report/tutors/index.php', array('id'=>$id));
$PAGE->set_pagelayout('report');

require_login($course);
$context = context_course::instance($course->id);
require_capability('report/tutors:view', $context);

// Trigger an activity report viewed event.
$event = \report_tutors\event\activity_report_viewed::create(array('context' => $context));
$event->trigger();
...
$stractivityreport = get_string('pluginname', 'report_tutors');
...
list($uselegacyreader, $useinternalreader, $minloginternalreader, $logtable) = report_tutors_get_common_log_variables();

// If no legacy and no internal log then don't proceed.
if (!$uselegacyreader && !$useinternalreader) {
    echo $OUTPUT->box_start('generalbox', 'notice');
    echo $OUTPUT->notification(get_string('nologreaderenabled', 'report_tutors'));
    echo $OUTPUT->box_end();
    echo $OUTPUT->footer();
    die();
}
...
$tutorstable->attributes['class'] = 'generaltable boxaligncenter';
$tutorstable->cellpadding = 5;
$tutorstable->id = 'tutorstable';
$tutorstable->head[] = $stractivity;

/* Pegar os nomes dos tutores */
 $tutorSQL = "SELECT u.id, concat(u.firstname,' ',u.lastname) fullname
                   FROM {role_assignments} ra
                   JOIN ({user} u, {context} cm, {course} c, {role} r) ON (r.archetype in ('coursecreator', 'manager', 'editingteacher')  and c.id = :courseid and u.id = ra.userid
...
if(count($tutorNames)>0){
	foreach($tutorNames as $tn){
		$tutorstable->head[] = $tn->fullname;
		$tutors[] = $tn->id;
		
	}
	$tutorStr = implode(',', $tutors);
}
    ...
    foreach ($section as $cmid) {
		...
		if(in_array($cmid, $show)){
			...
			if ($prevsecctionnum != $sectionnum) {
				...
				$sectioncell->colspan = count($tutorstable->head);
